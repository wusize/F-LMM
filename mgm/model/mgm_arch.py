...
class MGMMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            ...
            return input_ids, position_ids, attention_mask, past_key_values, None, labels, mask_ids, None
        ...
        mask_ids = [cur_mask_ids[cur_attention_mask] for cur_mask_ids, cur_attention_mask in zip(mask_ids, attention_mask)]
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            for i in range(len(image_token_indices) - 1):
                ...
                cur_mask_ids_noim.append(cur_mask_ids[image_token_indices[i] + 1:image_token_indices[i + 1]])
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    ...
                    cur_new_mask_ids.append(torch.full((cur_image_features.shape[0],), IMAGE_TOKEN_INDEX, device=cur_mask_ids.device, dtype=cur_mask_ids.dtype))
        ...
        new_mask_ids_padded = torch.full((batch_size, max_len), -1, dtype=new_mask_ids[0].dtype, device=new_mask_ids[0].device)
        attention_mask = torch.zeros((batch_size, max_len), dtype=attention_mask.dtype, device=attention_mask.device)
        position_ids = torch.zeros((batch_size, max_len), dtype=position_ids.dtype, device=position_ids.device)

        for i, (cur_new_embed, cur_new_labels, cur_new_mask_ids) in enumerate(zip(new_input_embeds, new_labels, new_mask_ids)):
        ...
        return None, position_ids, attention_mask, past_key_values, new_input_embeds, new_labels, new_mask_ids, image_places
