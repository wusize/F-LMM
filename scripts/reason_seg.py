...
def intersectionAndUnionGPU(output, target, K, ignore_index=255):
    ...
    area_intersection = intersection.sum()  # torch.histc(intersection, bins=K, min=0, max=K - 1)
    area_output = output.sum()  # torch.histc(output, bins=K, min=0, max=K - 1)
    area_target = target.sum()  # torch.histc(target, bins=K, min=0, max=K - 1)
